...
describe 'ユーザデータに対する操作', ->
  ...
  context "cron設定の日数をdecrementする", ->
    it "日付を減らす(日付が1日, 月が1月の場合についてもテスト)", ->
      expect(cron.decrementDayOfCronSetting("0 12 2 3 * *")=="0 12 1 3 * *").be.true
      expect(cron.decrementDayOfCronSetting("0 12 1 3 * *")=="0 12 29 2 * *").be.true
      expect(cron.decrementDayOfCronSetting("0 12 1 1 * *")=="0 12 29 12 * *").be.true
