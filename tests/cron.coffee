...
describe 'ユーザデータに対する操作', ->
  ...
  context "日時文字列 <=> cron設定 変換", ->
    it "日時の文字列をcronの設定に変換する", ->
      expect(cron.translateDateToCronSetting("2016-10-02") == "0 0 12 2 10 *").be.true
      expect(cron.translateDateToCronSetting("2016-01-12") == "0 0 12 12 1 *").be.true
  ...
  context "cron設定の日数をdecrementする", ->
    it "日付を減らす(日付が1日, 月が1月の場合についてもテスト)", ->
      ...
      expect(cron.decrementDayOfCronSetting("0 0 12 1 1 *")=="0 0 12 29 12 *").be.true
